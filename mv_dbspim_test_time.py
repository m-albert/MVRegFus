...
elastix_dir = '/home/malbert/software/fusion/dependencies_linux/elastix_linux64_v4.8'

# files to fuse
filepaths = ['/data/malbert/data/dbspim/chemokine/20170718_timer_wt_30hpf_time_lapse/20170718_timer_wt_30hpf_time_lapse(%s).czi' %i for i in range(1,30)]
...
# out_dir = '/data/malbert/regtest/time_noalign'
out_dir = '/data/malbert/regtest/time'
...
if __name__ == '__main__':
    ...
    # o = io_utils.get(graph,result_keys,local=False)
    # o = graph_multiview.client.get(graph,result_keys,local=False)
    ...
    for i in range(0,len(result_keys),N*len(channels)):
        graph_multiview.client.get(graph, result_keys[i:i+N*len(channels)], local=False)
        # io_utils.get(graph,result_keys[i:i+N*len(channels)],local=False)

    import imaris
    imaris.im_to_ims(fname=os.path.join(out_dir,'mv_tl.ims'),
                     filepattern=os.path.join(out_dir,'mv_000_%(t)03d_c%(c)02d.ims'),
                     channels=channels,tps=range(len(filepaths)),
                     copy_or_link='copy')
