...
def register_linear_elastix_old(fixed,moving,t0=None,elastix_dir='/scratch/malbert/dependencies_linux/elastix_linux64_v4.8'):
...
"""
    ...
    # fixed = (clahe(fixed,40,clip_limit=0.02)*10000).astype(np.uint16)
    # moving = (clahe(moving,40,clip_limit=0.02)*10000).astype(np.uint16)
    ...
    for i in range(1+int(np.trunc(np.log2(np.min(list(moving.shape)+list(fixed.shape))/10.))))[::-1]:
        f = int(2**i)
        factors.append(f)
        image_pyramid_line +=" %s %s %s" %(f,f,f)
        number_of_iterations_line +=" %s" %100
    image_pyramid_line += ")"
    number_of_iterations_line += ")"
    ...
    createParameterFile(np.array(fixed_spacing),elx_initial_transform_path, mod_params_affine, param_path_affine)
    ...
    # cmd = '%s -f %s -m %s -p %s -out %s -threads 1' %(elastix_bin,fixed_path,moving_path,param_path_affine,outdir)
    cmd = '%s -f %s -fMask %s -m %s -p %s -out %s -threads 1' %(elastix_bin,fixed_path,fixed_mask_path,moving_path,param_path_affine,outdir)
    if t0 is not None:
    ...
    # similarity_params   = get_affine_parameters_from_elastix_output(os.path.join(temp_dir,'TransformParameters.0.txt'))
    affine_params       = get_affine_parameters_from_elastix_output(os.path.join(temp_dir,'TransformParameters.0.txt'))
    # final_params = affine_params
    final_params = mult_aff(affine_params,params_to_matrix(t0))
    # out_param_file = h5py.File(filepath,'w')
    # out_param_file.clear()
    # out_param_file['prealignment'] = final_params
    # out_param_file['similarity'] = similarity_params
    # out_param_file['affine'] = affine_params
    # out_param_file.close()

    # shutil.copyfile(os.path.join(temp_dir,'TransformParameters.1.txt'),filepath)
    # else:
    #     final_params = h5py.File(filepath,'r')['prealignment'].value
    # params = get_affine_parameters_from_elastix_output(filepath,grid2world)

    return matrix_to_params(final_params)


def register_linear_elastix_seq(fixed,moving,t0=None,degree=2,elastix_dir='/scratch/malbert/dependencies_linux/elastix_linux64_v4.8'):
...
def register_nonlinear_elastix_seq(fixed,moving,t0=None,elastix_dir='/scratch/malbert/dependencies_linux/elastix_linux64_v4.8'):
    """
    assumes clahe images
    """
...
"""
    ...
    fixed = clahe(fixed,40,clip_limit=0.02)
    moving = clahe(moving,40,clip_limit=0.02)

    fixed = ImageArray(fixed,spacing=fixed_spacing,origin=fixed_origin)
    moving = ImageArray(moving,spacing=moving_spacing,origin=moving_origin)
    ...
    # for i in range(1+int(np.trunc(np.log2(np.min(list(moving.shape)+list(fixed.shape))/20.))))[::-1][-2:]:
    for i in [2]:
        f = int(2**i)
        factors.append(f)
        # image_pyramid_line +=" %s %s %s" %(f,f,2)
        image_pyramid_line +=" %s %s %s" %(2,2,2)
        number_of_iterations_line +=" %s" %1000
    ...
    for i in range(len(param_strings)):
        cmd += ' -p %s' %param_paths[i]
    cmd += ' -fMask %s' %fixed_mask_path
    cmd += ' -out %s -threads 1' %outdir

    cmd = cmd.split(' ')
    # subprocess.Popen(cmd,stdout=FNULL).wait()
    subprocess.Popen(cmd).wait()

    out_filepath = os.path.join(temp_dir,'TransformParameters.%s.txt' %(len(param_strings)-1))

    final_params = open(out_filepath).read()

    return final_params

def register_linear_elastix_20180228(fixed,moving):
    ...
    # print('warning: using easy mask')
    # m0 = (np.array(static > np.mean(static))).astype(np.uint16)
    # m1 = (np.array(mov > np.mean(mov))).astype(np.uint16)
...
def register_linear_elastix_ftrans(fixed,moving):
    ...
    reg_spacing = np.array([fixed.spacing[0]]*3)
    ...
    #                                 out_origin=static.origin,
    #                                 out_spacing=static.spacing
    #                                 )

    im0 = np.max(static_t,1)
    im1 = np.max(mov_t,1)

    import imreg

    offset = imreg.translation(im1,im0)
    offset = np.array([-offset[1],0,offset[0]]) * reg_spacing

    t0 = t00
    t0[9:] += offset
    mov_t2 = transform_stack_sitk(mov,
                                    t0,
                                    out_shape=stack_properties['size'],
                                    out_origin=stack_properties['origin'],
                                    out_spacing=stack_properties['spacing']
                                    )
    # tifffile.imshow(np.array([static_t,mov_t,mov_t2]).swapaxes(-3,-2),vmax=100)
    return t0
...
def register_linear_elastix_seq_y(fixed,moving,t0=None,elastix_dir='/scratch/malbert/dependencies_linux/elastix_linux64_v4.8'):
    ...
    static = ImageArray(fixed,spacing=fixed.spacing,origin=fixed.origin)
    mov = ImageArray(moving,spacing=moving.spacing,origin=moving.origin)
    ...
    return register_linear_elastix_seq(static,mov,t0,elastix_dir)

def get_offset_from_proj_registrations(im0,im1,t0=None):
    ...
    im1_t0 = transform_stack_dipy(im1,t0,out_shape=im0.shape,out_spacing=im0.spacing,out_origin=im0.origin)

    projs0 = [np.max(np.array(im0),dim) for dim in range(3)]
    projs1 = [np.max(np.array(im1_t0),dim) for dim in range(3)]

    translations = []
    for dim in range(3):
        origin0 = np.delete(im0.origin, dim, 0)
        origin1 = np.delete(im1.origin, dim, 0)
        spacing0 = np.delete(im0.spacing, dim, 0)
        spacing1 = np.delete(im1.spacing, dim, 0)

        # shape0 = np.array(projs0[dim].shape)
        # shape1 = np.array(projs1[dim].shape)
        # shape = np.max([shape0,shape1],0)
        # static = np.zeros(shape)
        # moving = np.zeros(shape)
        # static[:shape0[0],:shape0[1],:shape0[2]] = projs0[dim]
        # moving[:shape1[0],:shape1[1],:shape1[2]] = projs1[dim]
        # translation = np.array(imreg.translation(static,moving))

        indices = np.delete(range(3), dim, 0).astype(np.uint16)
        translation = np.zeros(3)
        translation[indices] = np.array(imreg.translation(projs0[dim],projs1[dim]))
        translations.append(translation)

    translations = np.array(translations)
    translations = np.ma.array(translations,mask=translations==0)
    print(translations)

    offset = np.mean(translations,0)
...
def register_warp_test(static,moving,t0=None):
    ...
    level_iters = list(np.array([1000000000]*2))
    # level_iters = list(np.array([0]*5))
    sigmas = [3,1]
    factors = [4,2]
    ...
    print('watch out, ss_sigma_factor and sigmas are being ignored!')
    ...
    out_params = translation.affine

    affine_params = np.zeros(12)
    affine_params[:9] = out_params[:3,:3].flatten()
    affine_params[9:] = out_params[:3,3]
    params_evolution = np.array([np.append(metric.params_evolution[i][:3,:3].flatten(),metric.params_evolution[i][:3,3]) for i in range(len(metric.params_evolution))])

    # out_params = np.array([[  2.66521879e-04,   0.00000000e+00,  -9.99999964e-01,
    #       7.31674137e+01],
    #    [  0.00000000e+00,   1.00000000e+00,   0.00000000e+00,
    #       2.43434502e-01],
    #    [  9.99999964e-01,   0.00000000e+00,   2.66521879e-04,
    #       3.13027644e+01],
    ...
    # from dipy.align.metrics import CCMetric_affine as CCMetric
    from dipy.align.metrics import CCMetric as CCMetric
    from dipy_helpers_test import register_dipy

    mapping = register_dipy(
                  np.array(static),
                  np.array(moving),
                  out_params,
                  scaling_factors = [8,4],
    ...
    # displ,A,c = linearise_displacement_field(mapping.backward,static)
...
def register_warp(static,moving,t0=None):
    ...
    mapping = register_dipy(
                  ...
                  np.array(moving),
                  out_params,
                  scaling_factors = [8,7,6,5,4,3,2,1],
                  sigmas_image = [8,7,6,5,4,3,2,1],
                  sigmas_defo = [20]*8,
                  clahe_kernel_sizes=None,
                  level_iters = [100]*8,
                  ccmetric_radius=4,
    ...
    # return params,[0]#metric.metric_evolution
    return params,[mapping,affine_params,params]

from sklearn import linear_model
def linearise_displacement_field(displ,weight_im=None):

    im = displ
    x_train = np.array([i.flatten() for i in np.mgrid[0:im.shape[0],0:im.shape[1],0:im.shape[2]]]).swapaxes(0,1)
    y_train = displ.reshape((np.product(displ.shape[:-1]),displ.shape[-1]))
    # A = np.array([2,0,0,0,1,0,0,0,1]).reshape((3,3))
    # c = np.array([0,0,0])
    # y_train = np.dot(A,x_train.swapaxes(0,1)).swapaxes(0,1) + c

    ols = linear_model.LinearRegression()

    if weight_im is not None:
        model = ols.fit(x_train, y_train, sample_weight=weight_im.flatten())
    else:
        model = ols.fit(x_train, y_train)

    linear_displ = model.predict(x_train).reshape(displ.shape)
    return linear_displ, model.coef_ + np.eye(3), model.intercept_
...
def transform_warp_dipy(stack,mapping=None,out_shape=None,out_spacing=None,out_origin=None):

    codomain_grid2world = np.diag(list(out_spacing) + [1])
    codomain_grid2world[:3,3] = out_origin

    domain_grid2world = np.diag(list(stack.spacing) + [1])
    domain_grid2world[:3,3] = stack.origin

    codomain_shape = np.array(out_shape)
    domain_shape = np.array(stack.shape)

    mapping.codomain_shape = codomain_shape.astype(np.int32)
    mapping.codomain_grid2world = codomain_grid2world.astype(np.float)
    mapping.codomain_world2grid = params_to_matrix(invert_params(matrix_to_params(codomain_grid2world))).astype(np.float)

    mapping.domain_shape = domain_shape.astype(np.int32)
    mapping.domain_grid2world = domain_grid2world.astype(np.float)
    mapping.domain_world2grid = params_to_matrix(invert_params(matrix_to_params(domain_grid2world))).astype(np.float)

    mapping.disp_shape = codomain_shape.astype(np.int32)
    mapping.disp_grid2world = codomain_grid2world.astype(np.float)
    mapping.disp_world2grid = params_to_matrix(invert_params(matrix_to_params(codomain_grid2world))).astype(np.float)

    res = mapping.transform(stack)
                            # image_world2grid=params_to_matrix(invert_params(matrix_to_params(domain_grid2world))).astype(np.float),
                            # out_shape = codomain_shape.astype(np.int32),
                            # out_grid2world = codomain_grid2world.astype(np.float),
                            # )

    return res.astype(stack.dtype)

def transform_warp_dipy2(stack,mapping=None,out_shape=None,out_spacing=None,out_origin=None,refim=None):

    image_world2grid = np.linalg.inv(get_grid2world(stack))

    if refim is None:
        out_grid2world = np.diag(list(out_spacing)+[1])
        out_grid2world[:3,3] = out_origin
    else:
        out_grid2world = get_grid2world(refim)
...
def register_linear_2d_pyelastix(im0,im1,t0=None,iterations=10,transform='rigid'):#,number_of_resolutions=4):
    ...
    os.environ['ELASTIX_PATH'] = '/home/malbert/software/fusion/dependencies_linux/elastix_linux64_v4.8'
    import pyelastix

    p_path = os.path.join(temp_dir,'p.txt')
    # p0_path = os.path.join(pyelastix.get_tempdir(),'p0.txt')

    if transform == 'translation':
        p = pyelastix.get_default_params('rigid')
        p.Transform = 'TranslationTransform'
    else:
        p = pyelastix.get_default_params(transform)

    p.AutomaticTransformInitialization = "false"
    p.InitialTransformParametersFileName = None
    p.MaximumNumberOfIterations = iterations
    p.FixedInternalImagePixelType = "float"
    p.MovingInternalImagePixelType = "float"
    p.ImageSampler = "Full"
    p.WriteResultImage = "false"
    min_pixels = 10 # amount of pixels the min downscaled dimension can have
    number_of_resolutions = 1 + int(np.trunc(np.log2(np.min(list(im0.shape)+list(im1.shape))/float(min_pixels))))
    p.NumberOfResolutions = number_of_resolutions
    ...
    # p.FinalGridSpacingInPhysicalUnits = 200
    # p.WriteResultImageAfterEachIteration = "true"

    # p0 = pyelastix.Parameters()
    # p0.Transform = "AffineTransform"
    # p0.HowToCombineTransforms = "Compose"
    # p0.InitialTransformParametersFileName = "NoInitialTransform"
    # p0.FixedImageDimension = 2
    # p0.MovingImageDimension = 2
    # p0.Transform = "AffineTransform"
    # p0.FixedInternalImagePixelType = "float"
    # p0.MovingInternalImagePixelType = "float"
    # p0.CenterOfRotationPoint = [0.,0.]
    # p0.Spacing = list(im1.spacing)
    # p0.Size = list(im1.shape)
    # p0.TransformParameters = list(t0)

    p = pyelastix._compile_params(p,im0)
    # write_pyelastix_parameter_file(p0.as_dict(),p0_path)
    write_pyelastix_parameter_file(p,p_path)

    fixed_path = os.path.join(temp_dir,'fixed.mhd')
    moving_path = os.path.join(temp_dir,'moving.mhd')

    fixedsitk = sitk.GetImageFromArray(im0)
    fixedsitk.SetSpacing(im1.spacing[::-1])
    fixedsitk.SetOrigin(im1.origin[::-1])
    movingsitk = sitk.GetImageFromArray(im1)
    movingsitk.SetSpacing(im1.spacing[::-1])
    movingsitk.SetOrigin(im1.origin[::-1])
    ...
    cmd = '%s -f %s -m %s' %(elastix_bin,fixed_path,moving_path)
    cmd += ' -p %s' %p_path
    # cmd += ' -t0 %s' %p0_path
    # cmd += ' -fMask %s' %fixed_mask_path
    # cmd += ' -out %s -threads 1' %temp_dir
    cmd += ' -out %s' %temp_dir
    ...
    final_params = t0
    for i in range(1):
        final_params = matrix_to_params(get_affine_parameters_from_elastix_output_2d(os.path.join(temp_dir,'TransformParameters.%s.txt' %i),t0=final_params))

    # m and f inverted!
    # im10,field = pyelastix.register(im1,im0,p)
    temp_dir_obj.cleanup()
    return params_to_matrix(final_params)

def register_linear_2d(im0,im1,t0=None):
    ...
    static_grid2world = get_grid2world(im0)
    moving_grid2world = get_grid2world(im1)
    ...
    translation = affreg.optimize(im0, im1, transform, params0,
                                  static_grid2world, moving_grid2world,
                                  starting_affine=starting_affine)
    params = translation.affine

    return params

def write_pyelastix_parameter_file(params,path):
