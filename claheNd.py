...
# import sys
# sys.path.append('/Users/malbert/Documents/projects/scikit-image')
import pdb
import numbers
import numpy as np
...
if __name__ == '__main__':
    ...
    # im = np.random.randint(0,100,((50,50))).astype(np.uint16)

    # im = np.zeros((50,50),dtype=np.uint16)
    # im[10:20,30:40] = np.random.randint(0,100,((10,10)))
    # im[10:20,30:40] = 10

    import SimpleITK as sitk
    # im = np.max(sitk.GetArrayFromImage(sitk.ReadImage('/data/malbert/atlas/data8/20140911_wt/atlas0_444_001_0.mhd')),0)
    # im = sitk.GetArrayFromImage(sitk.ReadImage('/data/malbert/atlas/data8/20140911_wt/atlas0_444_001_0.mhd'))
    # im = np.max(sitk.GetArrayFromImage(sitk.ReadImage('/Users/malbert/data/atlas/data8/20140911_wt/atlas0_444_001_0.mhd')),0)
    im = sitk.GetArrayFromImage(sitk.ReadImage('/Users/malbert/data/atlas/data8/20140911_wt/atlas0_444_001_0.mhd'))
    # im2 = np.max(sitk.GetArrayFromImage(sitk.ReadImage('/data/malbert/atlas/data8/20140911_mut/atlas0_444_001_0.mhd')),0)

    im = im[:50,:50,:50]

    # im = im[0:100:5,0:100:5]
    # cl = equalize_adapthist(np.pad(im,30,mode='edge'),kernel_size=6,clip_limit=0.02,nbins=2**13)[30:-30,30:-30,30:-30]
    # cls = []
    # for clip in np.linspace(0.01,0.5,10):
    #     print clip
    #     cls.append(equalize_adapthist(np.pad(im,30,mode='edge'),kernel_size=10,clip_limit=clip)[30:-30,30:-30,30:-30])

    # import clahe
    # cl2 = clahe.clahe(im,[6,6,6],clip_limit=0.02)

    import clahe_orig
    # from skimage import exposure
    import sys
    sys.path = ['/Users/malbert/Documents/projects/scikit-image/skimage'] + sys.path
    from skimage import exposure
    # import exposure as exposure2

    new = equalize_adapthist(np.max(im,0),5)
    orig = clahe_orig.equalize_adapthist(np.max(im,0),5)
    orig2 = exposure.equalize_adapthist(np.max(im,0),5)
    # orig3 = exposure2.equalize_adapthist(np.max(im,0),5)
